...
Перед подсчётом изменившихся пикселей применяется размытие радиусом два пикселя: оно подавляет различия
растеризации текста Chromium в Windows и Ubuntu, но сохраняет структурные изменения интерфейса.
